...
function mapFilterExport() {
  ...
  function generateMapWithoutBackground() {
    ...
    for (let i = 0; i < chunksAsArray.length; i++) {
      ...
      planetLocationsWithMark = planetLocationsWithMark.sort(
        (a, b) => a.mark - b.mark
      );
      ...
      for (let i = 0; i < planetLocationsWithMark.length; i++) {
        ...
        if (
          ...
          (i + 1 < planetLocationsWithMark.length &&
            planetLocationsWithMark[i].mark !=
              planetLocationsWithMark[i + 1].mark)
        ) {
          let newChunk = {};
  ...
  let onDownloadWithoutBackground = async () => {
    ...
    try {
      ...
      anchor.click();
      let newInfo = html`<div style=${{ color: "#FFFFCC" }}>
  ...
  async function processMap(input) {
    ...
    let newInfo = html`<div style=${{ color: "white" }}>
      Importing, this will take a while...
    </div>`;
    setInfo(newInfo);
    try {
      await df.bulkAddNewChunks(chunks);
      let newInfo = html`<div style=${{ color: "#FFFF00" }}>
  ...
  const leftLevelSelect = html`
    <select
      style=${selectStyle}
      value=${leftLevel}
      onChange=${(e) => setLeftLevel(e.target.value)}
    >
      ${PLANET_LEVELS.map(
  ...
  let thePlanetComponent = html`<div>
    <input
      type="checkbox"
      checked=${hasPlanet}
      onChange=${() => setHasPlanet(!hasPlanet)}
    />
    ${" Planet"}
  </div>`;
  ...
  let theFoundryComponent = html`<div>
    <input
      type="checkbox"
      checked=${hasFoundry}
      onChange=${() => setHasFoundry(!hasFoundry)}
    />
    ${" Foundry"}
  </div>`;
  ...
  let theQuasarComponent = html`<div>
    <input
      type="checkbox"
      checked=${hasQuasar}
      onChange=${() => setHasQuasar(!hasQuasar)}
    />
    ${" Quasar"}
  </div>`;
  ...
  let theBlueSpaceComponent = html`<div>
    <input
      type="checkbox"
      checked=${ohBlueSpace}
      onChange=${() => setOhBlueSpace(!ohBlackSpace)}
    />
    ${" Blue"}
  </div>`;
  ...
  return html`<div style=${divStyle}>
    ...
    <div style=${{ marginTop: "5px" }}>
      <button style=${buttonStyle} onClick=${updatePlanets}>
        Show Planets
      </button>
      ${" "}
      <button style=${buttonStyle} onClick=${clearPlanets}>
        Clear Planets
      </button>
    </div>
