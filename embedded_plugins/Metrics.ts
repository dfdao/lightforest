...
async function logPauseDifference(iterations = 1) {
  ...
  results["polling_interval"] = df
    .getEthConnection()
    .getProvider().pollingInterval;
  ...
  return html`
    <div style=${wrapperStyle}>
      ...
      <div style=${rowStyle}>
        ...
        <df-button onClick=${() => logPauseDifference()}>
          Log Pause Difference
        </df-button>
        <df-button onClick=${() => testPauseDifference()}>
          Test Pause Difference
        </df-button>
        <df-button onClick=${() => pollingIntervalSanityCheck()}>
