...
import { formatDate, formatStartTime } from "../../Utils/TimeUtils";
import {
...
export const MapDetails2: React.FC<MapDetailsProps> = ({
  configHash,
  config,
}) => {
  const numSpawnPlanets =
    config?.ADMIN_PLANETS.filter((p) => p.isSpawnPlanet).length ?? 0;
  console.log(numSpawnPlanets);
  ...
  useEffect(() => {
    async function loadMatches(configHash: string) {
      const matches = await loadLiveMatches(configHash);
      setLiveMatches(matches);
    }

    async function loadLeaderboards(
      configHash: string,
      numSpawnPlanets: number
    ) {
      const eloLeaderboard = await loadEloLeaderboard(
        configHash,
        numSpawnPlanets > 1
      );
      const arenaLeaderboard = await loadArenaLeaderboard(
        configHash,
        numSpawnPlanets > 1
      );
      console.log("elo", eloLeaderboard);
      console.log("arena", arenaLeaderboard);
      setEloLeaderboard(eloLeaderboard);
      setArenaLeaderboard(arenaLeaderboard);
    }
    setLiveMatches(undefined);
    setArenaLeaderboard(undefined);
    loadMatches(configHash);
    loadLeaderboards(configHash, numSpawnPlanets);
