...
function configItemName(config: LobbyInitializers, name: string) {
  ...
  const prettyName = capitalizeFirstLetter(
    name.toLowerCase().split("_").join(" ")
  );
  ...
  if (name == "CAPTURE_ZONE_CHANGE_BLOCK_INTERVAL")
    return "Capture Zone Change Frequency";
  if (name == "CAPTURE_ZONE_RADIUS") return "Capture Zone Radius";
  if (name == "CAPTURE_ZONE_PLANET_LEVEL_SCORE") return "Capture Zone Value";
  if (name == "CAPTURE_ZONE_HOLD_BLOCKS_REQUIRED")
    return "Capture Zone Hold Blocks";
  if (name == "CAPTURE_ZONES_PER_5000_WORLD_RADIUS")
    return "Capture Zone Rarity";
  ...
  const columns = [
    (item: any) => (
      <Cell style={{ fontWeight: "bold" }}>
        {configItemName(config, item[0].toString())}
      </Cell>
    ),
    (item: any) =>
