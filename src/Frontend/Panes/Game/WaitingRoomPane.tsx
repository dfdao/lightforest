...
export function WaitingRoomPane({
  visible,
  onClose,
}: {
  visible: boolean;
  ...
  useEffect(() => {
    ...
    const refreshPlayers = () => {
      ...
      spawnPlanets.forEach((planet) =>
        ps.push(uiManager.getPlayer(planet.owner))
      );
      setPlayers(ps);
    };
  ...
  const submitting = useMemo(() => {
    ...
    const submit =
      txs.hasTransaction(isUnconfirmedReadyTx) ||
      txs.hasTransaction(isUnconfirmedNotReadyTx);
    return submit;
  }, [
  ...
  function HelpContent() {
    return (
      <div>
        ...
        {confirmStart ? (
          <p>
            Once all players initialize and press READY, the game will begin.
          </p>
        ) : (
                  ...
                  <Btn
                    ...
                    onClick={async () => {
                      if (ready) {
                        ...
                        const eventDetails = getReadyEvent(
                          rct,
                          gameManager.getContract()
                        );
                        gameManager
                          .getContractAPI()
                          .emit(
                            ContractsAPIEvent.PlayerUpdate,
                            address(eventDetails.player)
                          );
