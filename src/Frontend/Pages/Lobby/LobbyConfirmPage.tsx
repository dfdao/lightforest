...
export function LobbyConfirmPage({
  ...
  }/play/${arenaCreationManager.getArenaAddress()}`;
  const twitters = useTwitters();
  const handleEnterUniverse = () => {
    if (
      config.ADMIN_PLANETS.displayValue &&
      config.ADMIN_PLANETS.displayValue.length > 0
    ) {
    ...
    if (
      config.WHITELIST.displayValue &&
      config.WHITELIST.displayValue.length > 0
    ) {
      const confirmed = confirm(
  ...
  const numStagedSpawnPlanets =
    config.ADMIN_PLANETS.displayValue?.filter((p) => p?.isSpawnPlanet).length ??
    0;

  const numStagedTargetPlanets =
    config.ADMIN_PLANETS.displayValue?.filter((p) => p?.isTargetPlanet)
      .length ?? 0;
  ...
  return (
    <Container>
      <Sidebar
        previousPath={!created ? root : undefined}
        title={!created ? "← Choose Map" : ""}
      >
        <span>
          Confirm your map configuration before creating your DF Arena Universe.
        </span>
        <Spacer height={24} />
        <PlanetListPane
          config={config}
      ...
      <MapContainer>
        ...
        {created ? (
          <span style={{ fontSize: "16px" }}>
            ...
            <span style={{ color: "#5CCDF0" }}>
              {numCreatedSpawnPlanets} player
              {numCreatedTargetPlanets == 1 ? "" : "s"}.
            </span>
            There {numCreatedTargetPlanets == 1 ? "is " : "are "}
            <span style={{ color: "#FF44B7" }}>
              {numCreatedTargetPlanets} target planet
              {numCreatedTargetPlanets == 1 ? "" : "s"}.
            </span>{" "}
            and{" "}
            <span style={{ color: "#E8E228" }}>
              {numCreatedSpawnPlanets} spawn planet
              {numCreatedSpawnPlanets == 1 ? "" : "s"}.
            </span>
          </span>
        ) : (
          <span style={{ fontSize: "16px" }}>
            ...
            <span style={{ color: "#5CCDF0" }}>
              {numStagedSpawnPlanets} player
              {numStagedSpawnPlanets == 1 ? "" : "s"}
            </span>{" "}
            ({totalStagedPlanets} planet{totalStagedPlanets == 1 ? "" : "s"}{" "}
            total). There {numStagedTargetPlanets == 1 ? "is " : "are "}
            <span style={{ color: "#FF44B7" }}>
              {numStagedTargetPlanets} target planet
              {numStagedTargetPlanets == 1 ? "" : "s"}
            </span>{" "}
            and{" "}
            <span style={{ color: "#E8E228" }}>
              {numStagedSpawnPlanets} spawn planet
              {numStagedSpawnPlanets == 1 ? "" : "s"}.
            </span>
            <br />
            <br />
            Once your world is created, your custom planets will automatically
            be created on-chain and you can enter the universe!
          </span>
        )}
          ...
          <>
            ...
            <Row>
              <CopyableInput
                ...
                displayValue={url}
                copyText={`👋 ${
                  twitters[arenaCreationManager.account] ||
                  arenaCreationManager.account.slice(0, 6)
        ...
        {!createDisabled && (
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <Button onClick={() => history.push(`${root}/settings/game`)}>
              Game Settings
            </Button>
            <Button onClick={() => history.push(`${root}/edit-map`)}>
              Add Planets
            </Button>
          </div>
        )}
