...
export function GameLandingPage({
  ...
  const [config, setConfig] = useState<LobbyInitializers>(
    stockConfig.competitive
  );
  const [creationManager, setCreationManager] =
    useState<ArenaCreationManager>();
    ...
    async (terminal: React.MutableRefObject<TerminalHandle | undefined>) => {
      if (!createInstance) {
        ...
        try {
          ...
          const { owner, lobby } = await newCreationManager.createAndInitArena(
            fetchedConfig
          );
  ...
  const advanceStateFromArenaCreated = useCallback(
    async (terminal: React.MutableRefObject<TerminalHandle | undefined>) => {
      ...
      try {
        if (!contractAddress || !creationManager)
      ...
      if (!useZkWhitelist) {
        ...
        if (!registerConfirmationResponse.txHash) {
          ...
          terminal.current?.printLink(
            ...
            () => {
              window.open(
                `${BLOCK_EXPLORER_URL}/${registerConfirmationResponse.txHash}`
              );
            },
            TerminalTextStyle.Blue
          );
  ...
  const advanceStateFromNoHomePlanet = useCallback(
    async (terminal: React.MutableRefObject<TerminalHandle | undefined>) => {
      ...
      const teamsEnabled = gameUIManager
        .getGameManager()
        .getContractConstants().TEAMS_ENABLED;
      const numTeams = gameUIManager
        .getGameManager()
        .getContractConstants().NUM_TEAMS;
