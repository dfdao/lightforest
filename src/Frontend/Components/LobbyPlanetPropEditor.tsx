...
const displayProperties: TitleInfo[] = [
  {
    title: "x",
    description: <span>The position of the planet on the x-axis</span>,
  },
  {
    title: "y",
    description: <span>The position of the planet on the y-axis</span>,
  },
  {
    title: "Level",
    description: (
      <span>
        Larger planets have more energy and can send resources farther. But they
        take more energy to capture.
      </span>
    ),
  },
  {
    ...
    description: (
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        <Sub>
          <White>Asteroid Fields</White> have half the defense of a planet, but
          produce silver.
        </Sub>
        <Sub>
          <White>Foundries</White> contain artifacts that can be discovered by
          players.
        </Sub>
        <Sub>
          <White>Spacetime Rips</White> withdraw and deposit artifacts and
          points.
        </Sub>
        <Sub>
          <White>Quasars</White> have no energy regen but store lots of energy
          and silver.
        </Sub>
      </div>
    ),
  },
  {
    ...
    description: (
      <span>
        If target planets are active, you must capture and fill a target planet
        with energy to win.
      </span>
    ),
  },
  {
    ...
    description: (
      <span>
        If spawn planets are active, players can only spawn on admin-created
        Spawn planets.
      </span>
    ),
  },
  {
    ...
    description: (
      <span>
        The player who initializes on a blocked spawn planet cannot move to this
        planet.
      </span>
    ),
  },
  ...
  function planetInput(value: planetInputType, index: number) {
    ...
    if (value == "x" || value == "y") {
      ...
      content = (
        ...
        <SelectFrom
          ...
          setValue={(value) =>
            onChange({ ...selectedPlanet, level: parseInt(value) })
          }
        />
      );
      ...
      content = (
        <SelectFrom
          ...
          setValue={(value) =>
            onChange({
              ...selectedPlanet,
              planetType: PLANET_TYPE_NAMES.indexOf(value),
            })
          }
        />
      );
      ...
      const options = stagedPlanets.reduce(
        (total, curr) =>
          curr.isSpawnPlanet
            ? [...total, { label: `(${curr.x},${curr.y})`, value: curr }]
            : total,
        []
      );
...

      {
        blockEnabled
                ...
                onChange={(selected: any) =>
                  onChange({
                    ...
                    blockedPlanetLocs: selected.map(
                      (block: { label: string; value: LobbyPlanet }) =>
                        block.value
                    ),
                  })
                }
          ...
          : (content = (
              <Hoverable onClick={() => history.push(`${root}/settings/game`)}>
                Enable
              </Hoverable>
            ));
      }
    ...
    return (
      <InputRow
        key={`input-row-${index}`}
        style={
          value == "blockedPlanetLocs" && blockEnabled
    ...
    <>
      {Object.keys(selectedPlanet).map(
        (value: planetInputType, index: number) =>
          !(excludePlanetTypes && excludePlanetTypes.includes(value)) &&
          planetInput(value, index)
      )}
    </>
  );
};
