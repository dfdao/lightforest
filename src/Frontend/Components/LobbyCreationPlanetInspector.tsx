...
export const LobbyCreationPlanetInspector: React.FC<LobbyPlanetInspectorProps> =
  ({
    ...
    const [mutablePlanet, setMutablePlanet] = useState<LobbyPlanet | undefined>(
      selectedPlanet
    );
    ...
    return (
      <Inspector>
        <InspectorInner>
          <HeaderRow>
            <InspectorTitle>
              {PlanetTypeNames[mutablePlanet.planetType]}
            </InspectorTitle>
            <CloseButton onClick={onClose} />
          </HeaderRow>
          <span style={{ maxWidth: "320px" }}>
            {PLANET_DESCRIPTION[mutablePlanet.planetType]}
          </span>
          <PlanetPropEditor
